...
impl<'a, Wr: MultipartWrite<Part> + ?Sized + Unpin, Part>
    Complete<'a, Wr, Part>
{
    pub(super) fn new(writer: &'a mut Wr) -> Self {
        Self { writer, is_terminated: false, _p: std::marker::PhantomData }
    }
}

impl<Wr: ?Sized + FusedMultipartWrite<Part> + Unpin, Part> FusedFuture
    for Complete<'_, Wr, Part>
{
